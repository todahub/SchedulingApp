...
export const AVAILABILITY_COMMENT_INTERPRETATION_SYSTEM_PROMPT = [
  ...
  "- 新しい relation を作らない",
  "- comparison を確定しない",
  "- preference_* や emotion_* は availability 本体として扱わない",
  "- availability 本体として扱ってよいのは availability_positive / availability_negative / availability_unknown のみ",
  '- 「行けるけど嫌」のような文では、「嫌」を availabilityTokenIndexes に入れない',
  "- relationContext / supportingContext は後段 comparison LLM のための材料であり、この段階で比較勝敗を決めない",
  "- JSON 以外を返さない",
  "",
  "今回使用可能な relation:",
  '- "applies_to"',
  ...
  '- targetContexts: targetTokenIndexes / relationContext? / supportingContext?',
  ...
  "",
  "5. targetContexts",
  "- targetContexts は任意。比較や選好の可能性がある文脈だけを残す",
  "- targetContexts.targetTokenIndexes は targetGroups 1 件と完全一致させる",
  "- relationContext / supportingContext には既存 targetGroupId / clauseGroupId / marker token index の参照だけを入れる",
  "- kind は contrast_availability_context / conditional_choice_scope / comparison_marker_scope / exception_or_residual_scope / none だけを使う",
  "- hint は comparison_candidate / preference_context / condition_context / none だけを使う",
  '- 「より」「の方が」「なら」「けど」「でも」「以外」などがあり、複数 target が関係するときだけ comparison_candidate を出してよい',
  '- 単なる「11なら行ける」のような条件付き可否では comparison_candidate を出さない',
  "- comparison を確定してはいけない。新しい targetGroup を作ってはいけない",
  "",
  "安全優先ルール:",
  "- 迷ったら relation を作らない",
...
export function buildAvailabilityCommentInterpretationUserPrompt(input: AvailabilityInterpretationExecutionInput) {
  return [
    ...
    "12. 比較候補を残したいときだけ targetContexts を使う",
    "13. targetContexts.targetTokenIndexes は targetGroups 1 件と完全一致させる",
    "14. relationContext / supportingContext には既存 targetGroupId / clauseGroupId / marker token index 以外を入れない",
    "15. comparison_candidate は複数 target と比較 marker がある場合だけに限定する",
    "",
    "clause ごとの決定済みヒント:",
...
export function buildAvailabilityCommentInterpretationRepairPrompt(args: {
  ...
  return [
    ...
    "- targetContexts を返すなら、targetTokenIndexes は targetGroups 1 件と完全一致させる",
    "- relationContext / supportingContext は既存 id / token index の参照だけを残す",
    "",
    "clause ごとの決定済みヒント:",
  ...
  return input.grouping.clauseGroups
    .map((clauseGroup) => {
      ...
      const lines = [
        ...
        `- ${clauseGroup.id}: contextTargetGroupIds=${JSON.stringify(clauseGroup.contextTargetGroupIds)}`,
      ];
