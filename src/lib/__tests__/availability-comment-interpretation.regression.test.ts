...
describe("availability comment auto interpretation", () => {
  ...
  it("stores structured condition interpretations in autoInterpretation.conditions", async () => {
    const candidates = buildDiscreteDayCandidates([11, 12]);
    const comment = "他の人がみんな行けるなら11がいい";
    const fetchMock = vi.fn().mockImplementation(async (_url: string, init?: RequestInit) => {
      const body = parseMockOllamaBody(init);
      const properties = body.format?.properties ?? {};
      ...
      if (isConditionInterpretationRequest(body)) {
        const conditionInput = parseStructuredInputFromUserPrompt<{
          finalPreferences: Array<{ targetText: string; targetTokenIndexes: number[] }>;
        }>(body);
        const targetTokenIndexes = conditionInput.finalPreferences[0]?.targetTokenIndexes ?? [];

        expect(conditionInput.finalPreferences[0]?.targetText).toBe("11");
          ...
          conditions: [
            {
              ...
              sourcePreferenceTargetTokenIndexes: targetTokenIndexes,
    ...
    expect(result.autoInterpretation.conditions).toEqual([
      expect.objectContaining({
        kind: "others_condition",
        resolverType: "all_others_available",
        participantScope: "all_others",
